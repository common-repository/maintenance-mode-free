    ...
    if( ! empty( $jsons ) ) {
      ...
      foreach ( $jsons as $path ) {
        ...
        if( is_object( $object ) ) {
          ...
          echo '<h4 class="cs-icon-title">'. __( 'Error! Can not load json file.', 'maintenance-mode-free' ) .'</h4>';
        }
...
if( ! function_exists( 'cs_set_icons' ) ) {
  function cs_set_icons() {
    ...
    echo '<div id="cs-icon-dialog" class="cs-dialog" title="'. __( 'Add Icon', 'maintenance-mode-free' ) .'">';
    echo '<div class="cs-dialog-header cs-text-center"><input type="text" placeholder="'. __( 'Search a Icon...', 'maintenance-mode-free' ) .'" class="cs-icon-search" /></div>';
    echo '<div class="cs-dialog-load"><div class="cs-icon-loading">'. __( 'Loading...', 'maintenance-mode-free' ) .'</div></div>';
    echo '</div>';
