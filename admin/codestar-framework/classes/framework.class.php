...
class CSFramework extends CSFramework_Abstract {
		...
		foreach ( $this->sections as $section ) {
			...
			if ( isset( $section['fields'] ) ) {
				...
				foreach ( $section['fields'] as $field_key => $field ) {
					...
					add_settings_field( $field_key . '_field', '', array(
						&$this,
						'field_callback'
					), $section['name'] . '_section_group', $section['name'] . '_section', $field );
	...
	public function validate_save( $request ) {
		...
		if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
			...
			$add_errors[] = $this->add_settings_error( __( 'Success. Imported backup options.', 'maintenance-mode-free' ), 'updated' );
		}

		// reset all options
		if ( isset( $request['resetall'] ) ) {
			$add_errors[] = $this->add_settings_error( __( 'Default options restored.', 'maintenance-mode-free' ), 'updated' );
		...
		if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
			...
			$add_errors[] = $this->add_settings_error( __( 'Default options restored for only this section.', 'maintenance-mode-free' ), 'updated' );
		}
	...
	public function admin_menu() {
		...
		if ( $args['menu_type'] == 'submenu' ) {
			call_user_func( 'add_' . $args['menu_type'] . '_page', $args['menu_parent'], $args['menu_title'], $args['menu_title'], $args['menu_capability'], $args['menu_slug'], array(
				&$this,
				'admin_page'
			) );
		} else {
			call_user_func( 'add_' . $args['menu_type'] . '_page', $args['menu_title'], $args['menu_title'], $args['menu_capability'], $args['menu_slug'], array(
				&$this,
				'admin_page'
			), $args['menu_icon'], $args['menu_position'] );
		}
	...
	public function admin_page() {
		...
		echo ( $this->settings['ajax_save'] ) ? '<span id="cs-save-ajax">' . __( 'Settings saved.', 'maintenance-mode-free' ) . '</span>' : '';

		submit_button( __( 'Save', 'maintenance-mode-free' ), 'primary cs-save', 'save', false, array( 'data-save' => __( 'Saving...', 'maintenance-mode-free' ) ) );
		submit_button( __( 'Restore', 'maintenance-mode-free' ), 'secondary cs-restore cs-reset-confirm', $this->unique . '[reset]', false );

		if ( $this->settings['show_reset_all'] ) {
			submit_button( __( 'Reset All Options', 'maintenance-mode-free' ), 'secondary cs-restore cs-warning-primary cs-reset-confirm', $this->unique . '[resetall]', false );
		}

		echo '</fieldset>';
		echo ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> ' . __( 'show all options', 'maintenance-mode-free' ) . '</a>' : '';
