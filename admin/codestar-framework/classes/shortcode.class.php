...
class CSFramework_Shortcode_Manager extends CSFramework_Abstract{
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      echo '<a href="#" class="button button-primary cs-shortcode" data-editor-id="'. $editor_id .'">'. __( 'Add Shortcode', 'maintenance-mode-free' ) .'</a>';
    }
  ...
  ?>
    <div id="cs-shortcode-dialog" class="cs-dialog" title="<?php _e( 'Add Shortcode', 'maintenance-mode-free' ); ?>">
      <div class="cs-dialog-header">
        <select class="<?php echo ( is_rtl() ) ? 'chosen-rtl ' : ''; ?>cs-dialog-select" data-placeholder="<?php _e( 'Select a shortcode', 'maintenance-mode-free' ); ?>">
          <option value=""></option>
          <?php
      ...
      <div class="cs-insert-button hidden">
        <a href="#" class="button button-primary cs-dialog-insert"><?php _e( 'Insert Shortcode', 'maintenance-mode-free' ); ?></a>
      </div>
    </div>
  <?php
  }
